#!/usr/bin/env python3
"""Normalize mode icons and map node states."""
...
SPECS = {
    "mode-mainline": (280, 280, 250, 250),
    "mode-daily": (280, 280, 250, 250),
    "mode-endless": (280, 280, 250, 250),
    "mode-advanced": (280, 280, 250, 250),
    "node-normal": (190, 190, 170, 170),
    "node-event": (190, 190, 170, 170),
    "node-reward": (190, 190, 170, 170),
    "node-boss": (230, 230, 210, 210),
}
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: normalize_modes_nodes.py <component-pack-v3-directory>")
    ...
    (base / "modes-nodes-manifest.json").write_text(json.dumps({"assets": records}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"normalized {len(records)} modes/nodes")
