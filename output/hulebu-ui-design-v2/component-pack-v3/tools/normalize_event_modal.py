#!/usr/bin/env python3
"""Normalize event cards, modal panels, and small state tags."""
...
SPECS = {
    "event-card": (340, 480, 315, 455),
    "upgrade-card": (340, 480, 315, 455),
    "pause-panel": (560, 700, 520, 660),
    "settings-panel": (560, 700, 520, 660),
    "tag-risk": (240, 90, 220, 72),
    "tag-reward": (240, 90, 220, 72),
}
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: normalize_event_modal.py <component-pack-v3-directory>")
    ...
    (base / "event-modal-manifest.json").write_text(json.dumps({"assets": records}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"normalized {len(records)} event/modal assets")
