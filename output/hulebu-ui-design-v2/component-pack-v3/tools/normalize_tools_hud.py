#!/usr/bin/env python3
"""Normalize tool buttons and runtime-driven HUD plaques."""
...
SPECS = {
    "tool-shuffle": (180, 180, 160, 160),
    "tool-undo": (180, 180, 160, 160),
    "tool-discard": (180, 180, 160, 160),
    "tool-vision": (180, 180, 160, 160),
    "hud-level": (320, 120, 300, 100),
    "hud-score": (320, 120, 300, 100),
    "hud-remaining": (420, 120, 400, 100),
    "hud-boss": (680, 120, 660, 100),
}
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: normalize_tools_hud.py <component-pack-v3-directory>")
    ...
    for name, (canvas_w, canvas_h, max_w, max_h) in SPECS.items():
        ...
        canvas.save(target)
        records.append({"name": name, "size": [canvas_w, canvas_h], "runtimeText": True})
    (base / "tools-hud-manifest.json").write_text(json.dumps({"assets": records}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"normalized {len(records)} tools/HUD assets")
