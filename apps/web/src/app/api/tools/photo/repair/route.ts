...
const localRepairUserId = "photo-editor-repair-preview";
...
export async function POST(request: Request) {
  try {
    ...
    if (!(image instanceof File) || typeof prompt !== "string") {
      return NextResponse.json({ error: "请上传图片并填写修复描述。" }, { status: 400 });
    }

    assertSupportedPhotoEditInput({ image, mode: "repair", prompt });
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "AI 细节修复任务创建失败，请稍后再试。" },
      { status: 500 }
    );
  }
}
