type MahjongComboType = "peng" | "chi" | "gang" | "hu";
type MahjongSuit = "wan" | "tiao" | "tong" | "honor";
type MahjongRank = 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9;
type MahjongTileLocation = "board";
...
const DEFAULT_OVERLAP_THRESHOLD = 0.10;
...
export function createMountainSkeleton(config: HulebuMountainGeneratorConfig): HulebuMountainSkeleton {
  ...
  const skeleton = buildBlockerGraph(nodes, {
  ...
  return enforceInitialFreeMinimum(skeleton, initialFreeRange.min);
}

function enforceInitialFreeMinimum(skeleton: HulebuMountainSkeleton, minFreeCount: number): HulebuMountainSkeleton {
  let freeCount = getInitialFreeNodeIds(skeleton).length;
  if (freeCount >= minFreeCount) {
    return skeleton;
  }

  const minX = Math.min(...skeleton.nodes.map((node) => node.x));
  const minY = Math.min(...skeleton.nodes.map((node) => node.y));
  const entranceColumns = 4;
  let releasedCount = 0;
  const candidates = skeleton.nodes
    .filter((node) => (node.blockedBy ?? []).length > 0)
    .sort((left, right) => left.layer - right.layer || (left.blockedBy?.length ?? 0) - (right.blockedBy?.length ?? 0));
  ...
  for (const node of candidates) {
    ...
    node.blockedBy = [];
    node.x = round(minX + (releasedCount % entranceColumns) * (node.width * 0.62));
    node.y = round(minY - node.height - Math.floor(releasedCount / entranceColumns) * (node.height * 0.22));
    node.layer = 0;
    for (const candidate of skeleton.nodes) {
      candidate.blocks = (candidate.blocks ?? []).filter((blockedId) => blockedId !== node.id);
      candidate.blockedBy = (candidate.blockedBy ?? []).filter((blockerId) => blockerId !== node.id);
    }
    releasedCount += 1;
    freeCount += 1;
  }
  ...
  return {
    ...
    metrics: {
      ...skeleton.metrics,
      initialFreeCount: getInitialFreeNodeIds(skeleton).length,
      coverEdgeCount: skeleton.nodes.reduce((total, node) => total + (node.blocks?.length ?? 0), 0),
    },
  };
}
