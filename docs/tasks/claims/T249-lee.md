...
- 禁止修改：Cocos 工程、正式资源包其他目录、玩法规则、Web 试玩版、M2 App Flow/存档、横屏、微信小游戏 SDK、PDF、AI 修图和其他游戏模块
- 验证命令：PNG 尺寸/可读性检查；人工视觉审阅；`npm run docs:sync`；`git diff --check`
- 当前阻塞：等待 Lee 审阅三张候选样稿。
- 下一步：根据审阅结论完成一次定向修正或关闭样稿批次；通过后为正式资源批次分配新任务。
