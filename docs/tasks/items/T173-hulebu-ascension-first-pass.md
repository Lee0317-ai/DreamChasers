...
- 负责人：Lee
- 状态：待验收
- 依赖：T165, T166, T167, T168, T169, T170, T171, T172
- 背景：T172 已完成成就图鉴第一版，但高阶周目仍未接入局外壳层。完整体验版下一步需要把主线通关后的更高难度轮回先做成一个可进入、可解释的第一版。
- 目标：
  - 在 `/games/hulebu` 局外页开放高阶周目第一版入口。
  - 至少支持 1-2 档高阶周目，并显示解锁状态。
  - 进入后会在外层摘要和内层 HUD 中显示当前周目与词缀/限制。
  - 周目第一版继续复用现有 20 关主线骨架与现有奖励池。
  - 同步站内静态 Demo、测试和模块文档。
- 不做：
  - 不做完整高阶事件扩容。
  - 不做完整 Boss 试炼第二版。
  - 不做云同步、排行榜、付费或广告。
  - 不改 Cocos 正式工程。
  - 不重做 20 关主线配置、每日 seed、无尽层数、奖励 effect 类型或局外升级价格。
...
- 拆分子任务：
  - [x] 写失败测试锁定高阶周目入口、周目字段和词缀文案。
  - [x] 改造局外壳层，开放高阶周目入口、解锁状态和周目摘要。
  - [x] 改造 HTML 原型，支持周目参数、周目 HUD 和第一版稳定词缀。
