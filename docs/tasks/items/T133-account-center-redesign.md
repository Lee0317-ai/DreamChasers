...
当前代码已有 T110-T113 的账号中心 MVP：邮箱验证登录、账号首页、权益账本、平台 API Key、产品 token exchange 和审计日志。2026-06-04 Lee 已修正后续实现口径：邮箱用于注册账号和验证邮箱，日常登录使用邮箱和密码。T108 已明确统一账号中心与 AI Gateway 总体方向，模型来源不能简化为平台直接保存用户模型 API Key。

## 目标

- 形成账号统一中心完整页面信息架构。
- 明确第一阶段改为邮箱注册验证 + 邮箱密码登录，并复用现有账号 MVP 能力。
