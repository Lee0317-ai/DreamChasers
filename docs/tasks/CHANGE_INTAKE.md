...
### IDEA-20260817-01：胡了卜堆叠方案由俯视改为侧视重力牌塔

- 提出人：Lee
- 提出时间：2026-08-17
- 背景：现有俯视"羊了个羊"式堆叠差异化有限。Lee 提供参考图（立牌金字塔 + 前后深度 + 侧边震落散牌区），希望整体改为侧视堆叠消除。已与 AI 逐项确认核心规则：重力垂直下落、仅列顶可点、深度只影响可见性、30-140 张压缩牌量、震落飞侧边散牌区。
- 目标：先在 Web 原型验证手感（T289），成立后再接调牌器与 Cocos；不成立则删除原型回退俯视，不影响现有主线。
- 不做：不修改现有主 demo、站内静态副本、Cocos 工程与共享包；不改关卡奖励数值；不在验证阶段做双视角并存。
- 用户价值：立牌坍塌带来更强爽感与辨识度，竖屏纵向空间利用率更高，震落机制表现力更强。
- 涉及模块：麻将 Roguelike 堆叠模型与表现层。
- 可能影响文件：`apps/game/mahjong-roguelike/prototypes/side-view/**`（新增）、任务分片与领取分片。
- 是否影响另一方任务：否，Lee 负责胡了卜全部工程。
- 是否需要新增任务：是
- 建议优先级：P1
- 验收标准：原型可完整试玩（点列顶入槽、吃碰杠胡消除、下落、震落散牌区）；Lee 自试手感通过；390px 竖屏无横向溢出。
- AI 初步方案：独立 `side-view/` 原型复用 v6 牌面资源与共享组合判定逻辑的简化版；金字塔轮廓生成器 + 列重力模型 + 侧边散牌区。
- 处理结论：已放弃（2026-08-17 Lee 决定）。两轮原型均与预期画面差距过大，主 demo 与 Cocos 保持俯视堆叠。详见 T289 分片。
- 对应任务编号：T289
...
# 2026-08-09：T252 胡了卜正式 UI 资源 Batch C

- 来源：Lee 要求继续生成正式图片资源。
- 变更类型：胡了卜正式 UI 内容层资源生产。
- 处理任务：T252。
- 范围：通过 PPTOKEN 新站生成无文字奖励卡、风场卡和教学/多候选/暂停/设置/结算弹窗母版；复用 v7 统一牌体输出 34 张正面和 1 张背面；扩展 formal v1 manifest 和校验报告。本批仍不接入 Cocos。

# 2026-08-09：T253 胡了卜正式字牌与背面透明底修正

- 来源：Lee 指出东南西北、中发白和背面存在白色底边，需要调整。
- 变更类型：正式资源 alpha 与采样边缘修复。
- 处理任务：T253。
- 范围：只修正 7 张字牌和 1 张背面的标准牌体 alpha 与透明像素 RGB，更新 Batch C 构建脚本、牌面预览、manifest 和校验报告；不修改其他数字牌或 Cocos 工程。

# 2026-08-09：T254 胡了卜正式字牌与背面底唇绿色化

- 来源：Lee 提供标注截图，指出 T253 后目标牌底部仍能看到浅色区域。
- 变更类型：正式资源牌体材质修正。
- 处理任务：T254。
- 范围：只把 7 张字牌和 1 张背面牌体内部的浅米色底唇改为绿色过渡，保留 T253 alpha/RGB 修复，不修改其他 27 张数字牌或 Cocos 工程。

# 2026-08-09：T255 胡了卜正式字牌与背面单层底座统一

- 来源：Lee 对比整套牌面截图，指出 T254 后东南西北、中发白和背面底部出现两层绿色，与万筒条的单层绿色底座不一致。
- 变更类型：正式资源牌体结构修正。
- 处理任务：T255。
- 范围：删除 T254 的额外底唇渐变，直接复用数字牌标准下半部牌体像素，使 7 张字牌和 1 张背面的底座轮廓、颜色、alpha 与数字牌一致；不修改其他 27 张数字牌或 Cocos 工程。

# 2026-08-09：T256 胡了卜正式字牌标准白色牌体统一

- 来源：Lee 指出 T255 后东南西北等字牌的白色区域仍分成两层，看起来像把整张旧牌图贴到标准牌体上。
- 变更类型：正式资源牌面结构修正。
- 处理任务：T256。
- 范围：以数字牌标准空白牌体作为 7 张字牌的完整底板，只提取并叠加东南西北、中发和白板边框内容；牌背保持独立绿色牌面；不修改其他 27 张数字牌或 Cocos 工程。

# 2026-08-09：T257 胡了卜中发白内容背景噪点清理

- 来源：Lee 在 T256 后继续提供标注截图，指出中、发、白以及相邻牌背区域仍有背景残留观感。
- 变更类型：正式字牌内容 mask 精修。
- 处理任务：T257。
- 范围：为中、发、白的内容 mask 增加连通域过滤，删除旧图纹理形成的离散背景噪点；通过 PPTOKEN 重新设计完整牌背母版，结构固定为连续青绿牌背面板加一层绿色底座，不再拉伸或染色旧下半部；不修改东南西北、其他 27 张数字牌或 Cocos 工程。

# 2026-08-10：T258 胡了卜正式 UI Batch D Cocos 接入

- 来源：Lee 确认正式资源可以使用，并要求放入 Cocos 正式工程。
- 变更类型：胡了卜 Cocos 正式表现层接入。
- 处理任务：T258。
- 范围：把 `hulebu-formal-ui-v1` 的背景、HUD、牌槽、动作/工具按钮三态、卡片、弹层和 35 张麻将牌完整导入 Cocos resources，切换运行时 SpriteFrame 映射并完成 `390×844` production 验收；不修改玩法规则、横屏或微信小游戏 SDK。

# 2026-08-10：T259 胡了卜 Cocos 竖屏正式 UI 布局重排

- 来源：Lee 提供 Cocos 正式 UI 截图，要求直接操作 Cocos 调整当前严重失衡的局内布局。
- 变更类型：胡了卜 Cocos 正式工程竖屏体验修复。
- 处理任务：T259。
- 范围：修正高像素密度竖屏下的逻辑尺寸换算，重新分配顶部 HUD、牌桌、工具列、动作栏和手牌槽的纵向空间；追加修正八条目录回退、不可点击暗态、放大牌点击命中，以及碰牌区到补杠识别的回归验证；只处理竖屏，不修改正式图片资源、玩法规则、关卡配置、横屏或微信小游戏 SDK。

# 2026-08-10：T260 胡了卜 Cocos 锁牌暗态与顶部 HUD 精修

- 来源：Lee 验收 T259 后指出下层不可点击牌仍呈亮色，分数牌匾与记牌器仍未完成。
- 变更类型：胡了卜 Cocos 正式工程竖屏视觉与状态反馈修复。
- 处理任务：T260。
- 范围：让被遮挡牌以明确暗态渲染且维持不可点击；重做分数牌匾的标题/数值排版和紧凑记牌器内容；补生产包竖屏视觉与交互回归。不修改玩法规则、关卡配置、正式图片原文件、横屏或微信小游戏 SDK。
# 2026-08-11：T261 胡了卜 Cocos 八条、牌体底层与动作按钮底色修复

- 来源：Lee 验收 T260 后指出八条牌面仍不正确、每张麻将底部仍多出一层、吃碰杠胡按钮底色与正式资源不一致。
- 变更类型：胡了卜 Cocos 正式 UI 资源与运行时叠层修复。
- 处理任务：T261。
- 范围：修正 formal v1 八条正式牌面或其 Cocos 映射；移除正式整牌 Sprite 后方多余程序化牌体；移除动作按钮正式状态图后方多余程序化底板并保持禁用/可用状态清楚。只处理竖屏正式 Cocos 表现，不修改玩法、关卡、存档、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T262 胡了卜 Cocos 分数、记牌器与锁牌牌背精修

- 来源：Lee 对 T261 production 截图验收后提出三项调整：移除分数数值后的浅色底块；记牌器紧凑态只显示“记牌器”，点击后展示全部麻将牌面及对应剩余数；不可点击牌改为显示正式牌背。
- 变更类型：胡了卜 Cocos 正式竖屏 HUD、记牌器和锁牌视觉调整。
- 处理任务：T262。
- 范围：修改 `GameSceneController.ts` 的分数/记牌器 UI、`BoardLayerBinder.ts` 的锁牌 Sprite 映射、必要的正式资源目录映射与共享回归测试；不修改玩法规则、关卡配置、点击判定、存档协议、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T263 胡了卜 Cocos 牌背点击穿透修复

- 来源：Lee 验收 T262 production 后发现点击不可点击牌背时，附近或上层可点击牌仍会被错误选中并落入手槽。
- 变更类型：胡了卜 Cocos 牌山输入命中修复。
- 处理任务：T263。
- 范围：修改 `BoardLayerBinder.ts` 的点击命中顺序，让光标下最上层实际牌先决定是否消费点击；牌背命中必须 no-op，不能穿透选择其他正面牌。补共享回归测试和任务文档；不修改牌山生成、关卡、HUD、存档、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T264 胡了卜 Cocos 通关弹层视觉优化

- 来源：Lee 验收正式竖屏 production 时指出通关后的 UI 仍需优化。
- 变更类型：胡了卜 Cocos 正式竖屏通关反馈优化。
- 处理任务：T264。
- 范围：为通关状态改用独立结算底板，整理标题、本层得分、关卡说明和继续按钮排版，并修正流程遮罩与右侧工具层的层级；不修改通关判定、奖励选择、关卡推进、计分规则、存档、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T265 胡了卜 Cocos 顶部 HUD、记牌器与已碰牌池优化

- 来源：Lee 对 T264 production 继续验收，指出记牌器展开位置、关卡牌匾底色、分数重复数字、余牌 UI、退出入口和已碰牌池仍需收口。
- 变更类型：胡了卜 Cocos 正式竖屏 HUD 与副露信息架构优化。
- 处理任务：T265。
- 范围：调整 `GameSceneController.ts` 的记牌器位置、顶部动态 HUD 和退出按钮；调整 `MeldRiverLayerBinder.ts` 的已碰牌池视觉；补共享回归测试和任务文档。不修改副露/补杠规则、计分、关卡、存档、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T266 胡了卜 Cocos 通关弹层标题避让

- 来源：Lee 提供 T265 production 通关截图，指出“第 2 层通关”仍被结算底图顶部莲花装饰遮住。
- 变更类型：胡了卜 Cocos 正式竖屏通关弹层排版修正。
- 处理任务：T266。
- 范围：只调整 `GameSceneController.ts` 通关弹层的面板高度与标题、得分、说明、继续按钮纵向坐标，补共享回归测试和任务文档；不修改通关流程、奖励、计分、其他 HUD 或正式原图。

# 2026-08-11：T267 胡了卜 Cocos 已碰牌池挂载与轻遮挡容差

- 来源：Lee 验收 T266 production 时反馈看不到已碰牌池，并明确轻微遮挡的牌仍应允许点击。
- 变更类型：胡了卜 Cocos 正式副露层挂载与牌山点击手感修正。
- 处理任务：T267。
- 范围：动态创建并挂载缺失的 `MeldRiverRoot`；把关卡生成层与 Board 点击层的覆盖锁定阈值从 `0.001` 统一调整为 `0.08`；补共享回归测试和任务文档。不修改副露/补杠数据、组合规则、关卡内容、HUD、存档、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T268 胡了卜完整局外与引导 UI 体系设计

- 来源：Lee 提出需要一整套登录、关卡选择、提示、教程和各种模式 UI 设计。
- 变更类型：胡了卜正式竖屏完整体验设计与交互原型。
- 处理任务：T268。
- 范围：以 `390×844` 竖屏为唯一设计基准，完成账号进入、标题大厅、模式选择、主线关卡地图、模式配置、局内教学、提示、暂停设置、奖励、胜负结算的统一信息架构和高保真交互稿；复用现有 formal-v1 的青玉、金红、深木视觉基线。本任务不修改 Cocos runtime、玩法规则、关卡数值、存档协议、Web Demo、横屏或微信小游戏 SDK，也不替代后续正式 PNG 切图与 Cocos Prefab 接入任务。

# 2026-08-11：T271 胡了卜完整局外流程正式视觉母稿

- 来源：Lee 确认继续推进 T268 后续正式视觉资源。
- 变更类型：胡了卜竖屏局外 UI 视觉母稿与资源接入规划。
- 处理任务：T271。
- 范围：基于 T268 交互稿和 formal-v1 视觉基线，生成登录大厅、模式选择、主线关卡地图、胜负结算四组 `390×844` 正式竖屏视觉母稿，并整理资源清单、Cocos key、状态和后续透明切图边界；不修改 Cocos runtime、玩法规则、关卡配置、存档、Web Demo 或既有 formal-v1 原图。

# 2026-08-11：T273 胡了卜标题与大厅透明组件资源包

- 来源：Lee 确认继续拆分 T271 已批准的第一批局外组件。
- 变更类型：胡了卜 Cocos 可接入透明 PNG 资源生产。
- 处理任务：T273。
- 范围：生成并裁切 `common/title/lobby` 第一批透明组件，建立 alpha、尺寸、锚点、九宫格、Cocos key 和来源追溯清单；不修改 Cocos runtime、玩法、关卡、存档、Web Demo、T269-T272 文件或既有 formal-v1/T271 母稿。

# 2026-08-11：T269 胡了卜 Cocos 多组合按钮上方候选菜单

- 来源：Lee 指出多组“碰”不应打开全屏弹框，而应在“碰”按钮上方出现两个候选。
- 变更类型：胡了卜 Cocos 局内组合二级选择交互修正。
- 处理任务：T269。
- 范围：把 `吃 / 碰 / 杠 / 补杠 / 胡` 多候选从全屏弹框改为对应组合按钮上方向上展开的局部菜单；保留现有候选状态机、精确候选执行和存档恢复。不修改组合判定、计分、副露、关卡、其他流程弹层、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T270 胡了卜 Cocos 已碰牌池、记牌器与组合候选精修

- 来源：Lee 验收 T269 production 时指出已碰牌池应位于左下角并点击按钮后展开，记牌器浅色面板辨识度不足，组合候选最多展示 3 个。
- 变更类型：胡了卜 Cocos 竖屏局内信息浮层与局部菜单精修。
- 处理任务：T270。
- 范围：调整 `MeldRiverLayerBinder.ts` 的已碰牌池入口和展开位置；调整 `GameSceneController.ts` 的记牌器展开配色和组合候选上限；补共享回归测试和任务文档。不修改副露/补杠数据、组合判定、计分、关卡、牌山、正式原图、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T272 胡了卜 Cocos 震落牌独立区域与静态圆点清理

- 来源：Lee 询问余牌下方四个静态点并确认删除，同时指出杠、补杠、胡产生的震落牌增多后会覆盖原牌山。
- 变更类型：胡了卜 Cocos 竖屏局内牌区信息架构修正。
- 处理任务：T272。
- 范围：删除顶部余牌下方无功能的四个静态点；在场景模型中区分原牌山与震落牌，把仍可点击的震落牌排入独立“震落牌区”；补共享回归测试和任务文档。不修改震落数量、组合规则、点击规则、计分、关卡、存档协议、正式原图、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T274 胡了卜模式与主线地图透明组件资源包

- 来源：Lee 要求继续完成胡了卜完整局外 UI 设计资源，T273 第一批标题/大厅组件已通过验收。
- 变更类型：胡了卜 Cocos 可接入透明 PNG 资源生产。
- 处理任务：T274。
- 范围：生成并裁切模式选择长卡、状态牌、五枚模式徽章，以及章节牌匾、星级牌匾、路径段、普通/当前/锁定/Boss 节点、空/实星和章节切换底栏；建立 alpha、尺寸、锚点、九宫格、Cocos key 和来源追溯清单。不修改 Cocos runtime、玩法、关卡、存档、Web Demo、既有 formal-v1、T271 母稿或 T273 组件包。

# 2026-08-11：T275 胡了卜胜负结算透明组件资源包

- 来源：Lee 要求继续完成胡了卜完整局外 UI 设计资源，标题/大厅与模式/地图两批组件已经通过验收。
- 变更类型：胡了卜 Cocos 可接入透明 PNG 资源生产。
- 处理任务：T275。
- 范围：生成并裁切胜利/失败印章、胜利/失败标题牌、统计牌匾、失败建议面板、解锁横幅和结算主/次按钮；建立 alpha、尺寸、锚点、九宫格、Cocos key 和来源追溯清单。不修改 Cocos runtime、玩法、关卡、存档、Web Demo、既有 formal-v1、T271 母稿、T273 或 T274 组件包。

# 2026-08-11：T276 胡了卜 Cocos 新手引导正式接入

- 来源：Lee 在正式局内 UI 验收时追问“我的新手关卡呢”，确认当前前五关只有教学牌局配置，没有可见的新手引导体验。
- 变更类型：胡了卜 Cocos 竖屏前五关教学流程接入。
- 处理任务：T276。
- 范围：在现有前五关牌局上叠加不遮挡主要操作的分步教学条，根据槽位和可执行组合动态更新提示，并在大厅增加不覆盖当前存档的“重玩新手”入口；不修改牌山生成、关卡数值、组合判定、计分、存档协议、正式原图、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-11：T277 胡了卜 Cocos 完整局外 UI 接入

- 来源：Lee 指定 Codex 任务 `019fefbb-39bb-71c0-a4f2-740c5e68b8b3`，要求把已经确认的完整局外 UI 接入当前 Cocos 游戏。
- 变更类型：胡了卜 Cocos 正式竖屏局外流程与胜负结算接入。
- 处理任务：T277。
- 范围：导入 T273-T275 的 40 个透明组件，替换启动页、大厅、模式选择、主线地图和胜负结算的旧通用弹层；复用当前存档、模式入口、新手教学与局内玩法，不修改牌山生成、关卡数值、组合判定、计分、存档协议、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-12：T278 胡了卜 Cocos 震落牌区空内容修复

- 来源：Lee 提供 production 截图，指出“震落牌区 4”只有数量和空面板，没有显示对应的 4 张震落麻将牌。
- 变更类型：胡了卜 Cocos 竖屏震落牌展示回归修复。
- 处理任务：T278。
- 范围：检查震落牌从运行时场景模型到 `MeldRiverLayerBinder` 的数据和渲染链路，让独立震落牌区按计数显示真实牌面并在数量增加时保持区域内排布；补共享回归测试、production 构建和竖屏浏览器验收。不修改震落数量、组合规则、点击判定、计分、关卡、存档协议、正式原图、Web Demo、横屏或微信小游戏 SDK。

# 2026-08-12：T279 胡了卜 Cocos 局内引导与大厅视觉修正

- 来源：Lee 提供局内与大厅截图，指出新手提示遮挡牌面、误入槽位的牌无法理解如何处理、牌河入口不清晰，以及大厅入口图被拉伸导致视觉质量差。
- 变更类型：胡了卜 Cocos 竖屏交互可理解性和大厅布局修正。
- 处理任务：T279。
- 范围：把新手提示移出牌山交互区；让牌河空槽、容量和打牌选择态可见；让槽位中的误选牌可以按明确流程打入河；把大厅四个正方形入口按原比例排成 2×2。不要修改关卡配置、规则、计分、Web Demo、其他模块或已有未提交 UI 资产。
- 结果：已完成，进入 T279 完成记录。

# 2026-08-12：T280 胡了卜 Cocos 杠候选与满槽河牌救场

- 来源：Lee 提供 `1-3` 截图，指出四张相同牌没有出现“杠”，槽位满后也没有明确失败提示或弃牌到牌河入口。
- 处理任务：T280。
- 范围：修复 Cocos 运行时四张同牌候选、把满槽状态明确为可河牌救场，并把右侧工具的视觉文案改为用户能理解的“河牌”；不修改关卡配置、规则配置、Web Demo 或正式资源。
# T281：通关结算提示优化

- 提出人：Lee
- 日期：2026-08-12
- 需求：优化通关后的提示层，避免宽窗口露出右侧牌局、文字压住装饰分栏，并保留继续按钮交互。
- 文件范围：Cocos 通关结算渲染、回归测试、任务文档。
- 验收：390×844 下内容完整居中；宽窗口下遮罩覆盖全视口；统计字段不重叠；继续按钮可触发后续流程。
# T282：河牌区限制为两格

- 提出人：Lee
- 日期：2026-08-12
- 需求：河牌区最多保留两个位置，避免和河牌区下方 UI 发生冲突。
- 文件范围：Cocos 河牌运行时容量、节点渲染和回归测试。
- 验收：河牌状态最多有两个节点；第三次弃牌不能进入河牌；画面不再创建第三个河牌格。
# T283：河牌救场 UI 说明与间距

- 提出人：Lee
- 日期：2026-08-12
- 需求：河牌区需要重新设计，右侧河牌按钮用途不清晰，且河牌格与下方槽位视觉冲突。
- 验收：按钮显示“弃牌”；河牌面板显示用途和操作步骤；河牌最多两格且与备用槽有清晰间距。
# T284：动作按钮兜底与河牌内容 UI

- 提出人：Lee
- 日期：2026-08-13
- 需求：修复可吃与弃牌按钮露出临时绿色方块的问题；河牌为空时不再显示两个空框。
- 验收：可用“吃”有正式兜底样式；“弃牌”用途清晰；河牌仅渲染实际已弃牌张数。
# 2026-08-13：T285 胡了卜弃牌交互 UI 设计资源

- 来源：Lee 要求设计一套牌局中的“弃牌”UI。
- 变更类型：胡了卜 Cocos 竖屏弃牌选择态、确认态与牌河反馈视觉设计。
- 处理任务：T285。
- 范围：设计弃牌入口、手牌选择态、底部确认条、牌河反馈和状态说明，生成可复用透明组件与接入说明；沿用现有 `formal-ui-v1` 的背景、手牌槽、麻将牌和字体动态渲染。
- 不修改：玩法判定、弃牌规则、河牌容量、关卡、存档、现有 runtime、现有正式资源包和 Web Demo。

# 2026-08-14：T287 胡了卜完整 UI 全新视觉方向概念图

- 来源：Lee 要求调用 generate2dmap 与 Grok 图片生成能力，为胡了卜完整 UI 重新设计全新视觉，并要求图片提示词与原资源完全隔离。
- 变更类型：胡了卜竖屏完整 UI 美术方向探索。
- 处理任务：T287。
- 范围：以相同牌局信息架构生成牌山奇谭、市井夜局、清爽新国潮三张方向概念图；图片、提示词、预览分别独立存放。
- 不修改：Cocos runtime、正式资源、玩法、关卡、存档、Web Demo 和既有 `output/hulebu-ui-assets/**`。

# 2026-08-14：T287 补充胡了卜登入与选关流程概念图

- 来源：Lee 确认局内玩法 UI 信息架构后，补充要求覆盖游戏登入过程和选择关卡流程。
- 变更类型：胡了卜竖屏局外流程与局内玩法统一视觉方向探索。
- 处理任务：继续并入 T287。
- 范围：新增启动/登入入口、模式选择、主线关卡地图、当前节点、奖励节点、事件节点、Boss 节点、通关/失败返回路径的概念图与独立提示词，并与局内牌山、记牌器、碰池、震落牌、河牌救场保持同一视觉系统。
- 不修改：Cocos runtime、正式资源、玩法、关卡、存档、Web Demo 和既有 `output/hulebu-ui-assets/**`。

# 2026-08-14：T288 优化 Grok 生图 CDN 下载与自动重试

- 来源：T287 生成过程中，PPTOKEN Grok API 已改为返回 `aiba-media.org` CDN，并间歇返回 `127.0.0.1` 回环地址；现有技能只信任 `pptoken.cc`，导致每次都需要人工诊断和临时下载。
- 变更类型：个人 Grok 图片生成技能兼容性修复。
- 处理任务：T288。
- 范围：扩展可信图片 CDN 白名单；自动过滤回环、私网和非 HTTPS 地址；对不可下载响应进行有限重试；新增离线单元测试和一次真实落盘验证。
- 不修改：游戏工程、正式资源、玩法、关卡、存档、Web Demo 和既有图片产物。

# 2026-08-14：T287 粉色胡萝卜吉祥物参考图调整

- 来源：Lee 提供粉色球形角色参考图，并确认其他 UI 方向暂时可接受。
- 变更类型：胡了卜完整 UI 统一吉祥物形象调整。
- 处理任务：继续并入 T287。
- 范围：参考圆润粉色球形轮廓、蓝色椭圆眼睛和黑色弯嘴，重新生成透明背景粉色胡萝卜母稿，并将其作为后续登入、选关、牌局、奖励、事件、Boss、失败和暂停图的统一角色标准。
- 不修改：Cocos runtime、正式资源、玩法、关卡、存档、Web Demo 和既有 UI 资源目录。
