...
- [x] manifest schema 5 写入 `sourceState: "clean"`、规范化 `sourceInputs`、源码/配置摘要、Creator provenance 与 `cocosTypecheckPassed: true`，校验器拒绝缺失或不匹配值。
...
- [ ] 在包含 checkpoint 与门禁提交的干净 worktree 运行 32 项工程测试、内建 Cocos TypeScript 检查的真实 production build 和 verify-only。
