...
- [x] 根工作区 Cocos TypeScript 检查退出码 0；干净 checkout 由 production wrapper 在 Creator 精确快照生成真实 declarations 后内建运行 `tsc --noEmit`。
...
- [x] manifest schema 6 写入 `sourceState: "clean"`、规范化 `sourceInputs`、源码/配置摘要、完整 Creator bundle provenance 与 `cocosTypecheckPassed: true`，校验器拒绝缺失或不匹配值。
...
- [x] focused 发布测试 189/189、Cocos 工程测试 32/32、Cocos TypeScript 与 `git diff --check` 已通过。
- [x] 已提交构建门禁：`569ef4d9`、`03455a1c`；独立评审后的 provenance/lifecycle 强化为 `2297f5d8`、`0725f31d`、`f2f69913`。
...
- [x] 在包含 checkpoint 与门禁提交的干净 worktree 运行 32 项工程测试、内建 Cocos TypeScript 检查的真实 production build 和 verify-only。
- [x] 启动本地静态 HTTP 并复核 5 个 smoke path 均为 200；manifest 对应 `f2f69913`、`sourceState: clean`、`566` 个文件、`135263593` 字节。
- [x] 在根工作区确认只有允许路径进入 T243 提交，无关工作区改动仍存在且未被覆盖。
- [x] 两轮独立代码评审均达到 0 Critical、0 Important；发现的 lifecycle/provenance 问题已修复并重跑相应验证。
- [x] 更新任务/领取/模块/进展/完成分片，运行 `npm run docs:sync`，但不把主摘要文件中的无关历史改动混入提交。
- [x] 删除临时 worktree；记录 T244 恢复 GameSession 组合命令边界。
