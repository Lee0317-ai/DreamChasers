...
账号中心应先完成统一信息架构和视觉重构，第一阶段采用邮箱注册验证 + 邮箱密码登录。AI 能力页先展示积分、订阅蓝图和 LLM 配置入口；第二阶段模型配置只落平台额度、临时 Key 和外部 Gateway BYOK，不保存用户 provider key 明文。真实模型调用、扣费、日志、provider 协议和失败切换统一归 AI Gateway 实现。
