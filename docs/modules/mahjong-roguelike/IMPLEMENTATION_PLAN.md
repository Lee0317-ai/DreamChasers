...
- 特殊事件池扩容：事件稀有度、正负事件、路线联动和高阶事件。第一轮已完成 `稀有事件 / 构筑联动 / 高阶事件` 文案层和第二版事件池。
...
- 按 T175 路线先把 `/games/hulebu` 网页版内容、系统、数值和长期目标稳定，再追平 Cocos 和音画资源。
...
- `东风场 / 南风场 / 西风场 / 北风场` 四档高阶入口。
- 局外 `高阶配置` 能力面板和能力槽限制。
- 内层试玩页高阶能力效果、专属奖励、档位事件、构筑识别、失败复盘和结算复盘。
- 北风场 `封盘 / 迟火` 能力与 `封终流` 构筑线。
- Boss 阶段目标池、普通/终局/高阶/无尽 Boss 变体、`Boss 奖励品质` 和外层 `Boss 复盘` 结算卡。
- 站内静态 Demo 和文本回归测试同步。
...
## 9. 阶段 7：Cocos 旧工程复用和正式追平

目标：

- 在 Web 规格冻结后，复用已有 Cocos Creator 3.8.8 工程作为正式版本基座，逐步追平当前 Web 玩法和视觉。

当前状态：

- T189 已立项并完成复用边界梳理。
- 旧 Cocos 工程位于 `apps/game/mahjong-roguelike/cocos/hulebu-cocos-3.8.8/`。
- 工程已有项目壳、场景 Binder、runtime、牌面资源加载、20 关轻量流程、随机牌山、遮挡点击和共享测试。

推荐顺序：

1. 视觉资源同步：同步 Web 当前 v6 牌面、按钮、卡槽和操作 UI，检查全量牌面映射。
2. 核心局内规则追平：补有限牌河、明牌组、补杠、震落、满槽救场和提示。
3. 内容系统追平：接 Boss 第二版、特殊事件、无尽、每日、高阶和局外成长。
4. 发布准备：补音效、动效、图集、加载、移动端性能和包体检查。

首个 Cocos 代码任务建议验收：

- Cocos 首屏视觉对齐 Web 当前牌面和按钮。
- 点击入槽、组合消除、遮挡解锁不回退。
- `npm run test -w packages/shared -- mahjong-cocos-project` 和 Cocos `tsc` 通过。
