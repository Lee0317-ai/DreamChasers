...
T101 已把 T100 的第一版核心规则落进默认玩家 Demo，T102 已把该 Demo 接到 `/games/hulebu`，T105 已修正震落牌平铺和遮挡点击。下一轮先让 Lee 和朋友试玩站内发布版，重点观察：`3` 格牌河是否过松或过紧、补杠是否有效消化孤张但不抢明杠爽点、`杠` 震落 1 张和 `胡` 震落 3 张是否形成合适恢复强度。第 5 关已保证至少 2 条干净明杠路线，不应再只靠 3 张组推进；试玩时还要确认震落牌不会再次叠起、被盖住普通牌不能点击、`听/差` 提示和上下两层记牌器是否足够清楚。`悬台窄腰` 已先接入调牌器，验收地址为 `/games/hulebu-demo/tuner.html?template=suspended-waist&level=10&seed=waist-check`；试玩反馈稳定后，再另开任务决定是否加入默认高压关 auto 池、补残局收官整合、提示表现增强，以及是否沉淀到 Cocos 正式工程。
