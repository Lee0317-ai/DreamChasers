...
T101 已把 T100 的第一版核心规则落进默认玩家 Demo，T102 已把该 Demo 接到 `/games/hulebu`，T105 已修正震落牌平铺和遮挡点击，T162 已补上高压窄腰池和残局收官第一阶段，T163 已补上特殊事件第一版，T164 已补上第 10 关终局试炼第一版。下一轮先让 Lee 和朋友试玩站内发布版，重点观察：`3` 格牌河是否过松或过紧、补杠是否有效消化孤张但不抢明杠爽点、`杠` 震落 1 张和 `胡` 震落 3 张是否形成合适恢复强度、`残局收官` 的弃牌和牌引是否有决策感、特殊事件触发节奏和主动加压收益是否清楚、终局试炼 `杠 1 / 胡 1 / 积分 180` 是否既有压力又不会卡死。第 5 关已保证至少 2 条干净明杠路线，不应再只靠 3 张组推进；试玩时还要确认震落牌不会再次叠起、被盖住普通牌不能点击、`听/差` 提示、终局试炼紧凑目标条和上下两层记牌器是否足够清楚。
特殊事件第一版已落地，后续如果继续扩展高阶挑战，建议另开任务规划 Boss 试炼、事件池扩容、事件奖励品质、卡槽压缩和完整高阶周目，不要直接在 Demo 收尾补丁里继续堆。
