...
T276 已把此前只有配置的前五关教学补成可见运行时体验。`TutorialGuidePanel` 挂在 `ToolOverlayRoot`，根据 `currentDisplayLevelOrder`、已占槽位和可执行组合生成动态文案；它没有 `BlockInputEvents`，展开记牌器或进入流程弹层时自动隐藏。大厅 `LobbyMode_Tutorial` 会启动 `tutorialReplayMode`，该模式的 `commitActiveRun()` 直接返回且不覆盖原存档，第五关完成或中途退出后回大厅重新加载原 active run。后续不要把教学重玩改成先清空 active run，也不要给教学条增加全屏遮罩。production build ID 为 `dec5351f0057-20260811T144015Z`。
