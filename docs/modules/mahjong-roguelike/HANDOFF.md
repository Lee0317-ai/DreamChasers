...
- Lee 已确认正式发布方向按微信小程序常见竖屏体验验收，横屏不属于当前目标，不登记横屏适配任务，也不修改 Binder/UI/layout。T244 当前待验收；既有 `390x844` 正式包证据覆盖完整 UI 可见、选牌与刷新恢复，仍需补一次组合、多候选 exact choice 和清关 smoke。
- 框架规划完成。
- 模块文档目录已建立。
